...
def main():
    # 모델명
    model = 'bert_model2.pt'
    model = MobileBertForSequenceClassification.from_pretrained(model)
    model.eval()
    # Test
    df = pd.read_csv("review_data.csv", encoding="UTF-8")
    data_X = df['reviewText'].values
    labels = df['label'].values

    tokenizer = MobileBertTokenizer.from_pretrained('mobilebert-uncased', do_lower_case=True)
    input_ids = [tokenizer.encode(str(x), add_special_tokens=True) for x in data_X]
